"""
Copied from torch/fx/passes/graph_drawer.py
"""
    ...
    class FxGraphDrawer:
        ...
        def _get_node_label(
            ...
            if node.op == "call_module":
                ...
                label += self._typename(leaf_module) + r"\n|"
                extra = "{attr:" + r"\l"
                ...
                if not isinstance(leaf_module, torch.fx.GraphModule):
                    named_params = list(leaf_module.named_parameters())
                    named_bufs = list(leaf_module.named_buffers())

                    has_leaf = len(named_params) > 0
                    if has_leaf:
        ...
        def _to_dot(
            ...
            for node in graph_module.graph.nodes:
                ...
                label = self._get_node_label(graph_module, node, skip_node_names_in_args)                    
